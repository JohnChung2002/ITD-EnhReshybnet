...
def combine_by_timerange_pandas(dname = 'r4.2'):
    ...
    while sum(stop.values()) < 5:
        ...
        thisweekdf.to_pickle("DataByWeek-homo/"+str(week_index)+".pickle")
        week_index += 1
...
def getuserlist(dname = 'r4.2', psycho = True):
    ...
    w1 = pd.read_pickle("DataByWeek-homo/1.pickle")
    w2 = pd.read_pickle("DataByWeek-homo/2.pickle")
...
def process_week_num(week, users, userlist = 'all', data = 'r4.2'):
    ...
    acts_week = pd.read_pickle("DataByWeek-homo/"+str(week)+".pickle")
    ...
    df_u_week.to_pickle("NumDataByWeek-homo/"+str(week)+"_num.pickle")
...
def to_csv(week, mode, data, ul, uf_dict, list_uf, subsession_mode = {}):
    ...
    w = pd.read_pickle("NumDataByWeek-homo/"+str(week)+"_num.pickle")
    ...
    towrite.to_pickle("tmp-homo/"+str(week) + mode+".pickle")
...
if __name__ == "__main__":
    ...
    [os.mkdir(x) for x in ["tmp-homo", "ExtractedData-homo", "DataByWeek-homo", "NumDataByWeek-homo"]]
    ...
    all_csv = open('ExtractedData-homo/'+mode+dname+'.csv','a')
    
    towrite = pd.read_pickle("tmp-homo/"+str(weekRange[0]) + mode+".pickle")
    towrite.to_csv(all_csv,header=True, index = False)
    for w in weekRange[1:]:
        towrite = pd.read_pickle("tmp-homo/"+str(w) + mode+".pickle")        
        towrite.to_csv(all_csv,header=False, index = False)
                
    print(f'Extracted {mode} data. Time (mins): {(time.time()-st)/60:.2f}')
    st = time.time()

    [os.system(f"rm -r {x}") for x in ["tmp-homo", "DataByWeek-homo", "NumDataByWeek-homo"]]
