...
def combine_by_timerange_pandas(dname = 'r4.2'):
    ...
    while sum(stop.values()) < 5:
        ...
        for act in allacts:
            ...
            thisweekdf = thisweekdf._append(df, sort=False)
        
        thisweekdf['date'] = thisweekdf['date'].apply(lambda x: datetime.strptime(x, "%m/%d/%Y %H:%M:%S"))
        thisweekdf.to_pickle("DataByWeek-comb/"+str(week_index)+".pickle")
        week_index += 1
...
def getuserlist(dname = 'r4.2', psycho = True):
    ...
    w1 = pd.read_pickle("DataByWeek-comb/1.pickle")
    w2 = pd.read_pickle("DataByWeek-comb/2.pickle")
...
def process_week_num(week, users, userlist = 'all', data = 'r4.2'):
    ...
    acts_week = pd.read_pickle("DataByWeek-comb/"+str(week)+".pickle")
    ...
    df_u_week.to_pickle("NumDataByWeek-comb/"+str(week)+"_num.pickle")
...
def f_calc(ud, mode = 'week', data = 'r4.2'):
    ...
    all_countonlyf = {'pc':[0,1,2,3]} if mode != 'session' or mode != 'activity' else {}
    ...
    logon_countonlyf = {'pc':[0,1,2,3]} if mode != 'session' or mode != 'activity' else {}
    logon_statf = []
    ...
    device_countonlyf = {'pc':[0,1,2,3]} if mode != 'session' or mode != 'activity' else {}
    ...
    if mode != 'session' or mode != 'activity': file_countonlyf = {'to_usb':[1],'from_usb':[1], 'file_act':[1,2,3,4], 'disk':[0,1], 'pc':[0,1,2,3]}
    ...
    if mode != 'session' or mode != 'activity': mail_countonlyf = {'Xemail':[1],'exbccmail':[1], 'pc':[0,1,2,3]}
    ...
    if mode == 'session' or mode == 'activity': http_count_subf.pop('pc',None)
    ...
    elif mode == 'session' or mode == 'activity':
...
def activity_calc(ud, week, mode, data, uw, v, list_uf):
    d = ud.iloc[0]['day']
    perworkhour = sum(ud['time']==1)/len(ud)
    perafterhour = sum(ud['time']==2)/len(ud)
    perweekend = sum(ud['time']==3)/len(ud)
    perweekendafterhour = sum(ud['time']==4)/len(ud)
    timestamp = ud.iloc[0]['time_stamp']
    time = timestamp.timestamp()
    
    tmp = f_calc(ud, mode, data)
    activity_instance = [time, v, d, week, ud.iloc[0]['pc'], perworkhour, perafterhour, perweekend, perweekendafterhour] + \
        (uw.loc[v, list_uf + ['ITAdmin', 'O', 'C', 'E', 'A', 'N'] ]).tolist() + tmp[2] + [tmp[4]]
    return (activity_instance, tmp[3])
...
def to_csv(week, mode, data, ul, uf_dict, list_uf, subsession_mode = {}):
    ...
    if mode == 'session': 
        ...
        cols2a = ['timestamp','user', 'day', 'week', 'pc', 'isworkhour', 'isafterhour', 'isweekend', 'isweekendafterhour'] + list_uf + ['ITAdmin','O','C','E','A','N']
    else: cols2a = ['starttime', 'endtime','user','week'] + list_uf + ['ITAdmin','O','C','E','A','N']
    cols2b = ['insider']        

    w = pd.read_pickle("NumDataByWeek-comb/"+str(week)+"_num.pickle")
    ...
    for v in user_dict:
        if v in usnlist:
            ...
            if mode == 'activity':
                for index in range(len(uactw)):
                    ud = uactw.iloc[[index]]
                    if len(ud) > 0:
                        activity_instance, i_fnames = activity_calc(ud, week, mode, data, uw, v, list_uf)
                        towrite_list.append(activity_instance)


    towrite = pd.DataFrame(columns = cols2a + i_fnames + cols2b, data = towrite_list)
    towrite.to_pickle("tmp-comb/"+str(week) + mode+".pickle")
    
    if mode == 'session' and len(subsession_mode) > 0:
        for k1 in subsession_mode:
            for k2 in subsession_mode[k1]:
                df_tmp = pd.DataFrame(columns = ['subs_ind']+cols2a + i_fnames + cols2b, data = towrite_list_subsession[k1][k2])
                df_tmp.to_pickle("tmp-comb/"+str(week) + mode + k1 + str(k2) + ".pickle")
...
if __name__ == "__main__":
    ...
    [os.mkdir(x) for x in ["tmp-comb", "ExtractedData-comb", "DataByWeek-comb", "NumDataByWeek-comb"]]
    ...
    for mode in ['day', 'activity']:
    
        weekRange = list(range(0, numWeek)) if mode in ['day', 'session', 'activity'] else list(range(1, numWeek))
        ...
        all_csv = open('ExtractedData-comb/'+mode+dname+'.csv','a')
        
        towrite = pd.read_pickle("tmp-comb/"+str(weekRange[0]) + mode+".pickle")
        towrite.to_csv(all_csv,header=True, index = False)
        for w in weekRange[1:]:
            towrite = pd.read_pickle("tmp-comb/"+str(w) + mode+".pickle")        
            towrite.to_csv(all_csv,header=False, index = False)
        
        if mode == 'session' and len(subsession_mode) > 0:
            for k1 in subsession_mode:
                for k2 in subsession_mode[k1]:
                    all_csv = open('ExtractedData-comb/'+mode+ k1 + str(k2) + dname+'.csv','a')
                    towrite = pd.read_pickle('tmp-comb/'+str(weekRange[0]) + mode + k1 + str(k2)+".pickle")
                    towrite.to_csv(all_csv,header=True, index = False)
                    for w in weekRange[1:]:
                        towrite = pd.read_pickle('tmp-comb/'+str(w) + mode+ k1 + str(k2)+".pickle")        
                        towrite.to_csv(all_csv,header=False, index = False)
                    
        print(f'Extracted {mode} data. Time (mins): {(time.time()-st)/60:.2f}')
        st = time.time()

    [os.system(f"rm -r {x}") for x in ["tmp-comb", "DataByWeek-comb", "NumDataByWeek-comb"]]
    # [os.system(f"rm -r {x}") for x in ["tmp-comb", "DataByWeek-comb", "NumDataByWeek-comb", "ExtractedData-comb"]]