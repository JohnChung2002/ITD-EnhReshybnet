...
def combine_by_timerange_pandas(dname = 'r4.2'):
    ...
    while sum(stop.values()) < 5:
        ...
        thisweekdf.to_pickle("DataByWeek-hetero/"+str(week_index)+".pickle")
        week_index += 1
...
def getuserlist(dname = 'r4.2', psycho = True):
    ...
    w1 = pd.read_pickle("DataByWeek-hetero/1.pickle")
    w2 = pd.read_pickle("DataByWeek-hetero/2.pickle")
...
def process_week_num(week, users, userlist = 'all', data = 'r4.2'):
    ...
    acts_week = pd.read_pickle("DataByWeek-hetero/"+str(week)+".pickle")
    ...
    df_u_week.to_pickle("NumDataByWeek-hetero/"+str(week)+"_num.pickle")
...
def f_calc(ud, data = 'r4.2'):
    ...
    file_countonlyf = {'to_usb':[1],'from_usb':[1], 'file_act':[1,2,3,4], 'disk':[0,1], 'pc':[0,1,2,3]}
...
def combine_all_actions(dname='r4.2'):
    # Define your sources and (if needed) columns per source.
    allacts = ['device','email','file','http','logon']
    # You will build a list of dictionaries (one per action).
    all_actions = []
    
    for act in allacts:
        filename = act + '.csv'
        with open(filename, 'r') as f:
            header_line = next(f).strip()  # skip header row
            # Determine columns as in your original code:
            if act == 'email':
                if dname in ['r4.1','r4.2']:
                    columns = ['id', 'date', 'user', 'pc', 'to', 'cc', 'bcc', 'from', 'size', '#att', 'content']
                else:  # for r5.1, r5.2, r6.1, r6.2
                    columns = ['id', 'date', 'user', 'pc', 'to', 'cc', 'bcc', 'from', 'activity', 'size', 'att', 'content']
            elif act == 'logon':
                columns = ['id', 'date', 'user', 'pc', 'activity']
            elif act == 'device':
                ...
                columns = None  # adjust if necessary
            ...
            for line in f:
                ...
                if not line: 
                    continue
                # (This part mimics your handling of quotes for some files.)
                if dname in ['r6.1','r6.2'] and act in ['email', 'file', 'http'] and '"' in line:
                    firstpart = line[:line.find('"')-1]
                    content = line[line.find('"')+1:-1]
                    parts = firstpart.split(',') + [content]
                else:
                    parts = line.split(',')
                if len(parts) < len(columns):
                    # Handle or skip malformed rows.
                    continue
                # Create a dictionary for this row.
                row_dict = dict(zip(columns, parts))
                row_dict['type'] = act  # record the source type
                all_actions.append(row_dict)
    
    # Create a DataFrame from all actions
...
def to_csv(week, mode, data, ul, uf_dict, list_uf, subsession_mode = {}):
    ...
    w = pd.read_pickle("NumDataByWeek-hetero/"+str(week)+"_num.pickle")
    ...
    towrite.to_pickle("tmp-hetero/"+str(week) + mode+".pickle")
...
if __name__ == "__main__":
    ...
    [os.mkdir(x) for x in ["tmp-hetero", "ExtractedData-hetero", "DataByWeek-hetero", "NumDataByWeek-hetero"]]
    ...
    weekRange = list(range(0, numWeek))
    ...
    all_csv = open('ExtractedData-hetero/'+mode+dname+'.csv','a')

    towrite = pd.read_pickle("tmp-hetero/"+str(weekRange[0]) + mode+".pickle")
    towrite.to_csv(all_csv,header=True, index = False)
    for w in weekRange[1:]:
        towrite = pd.read_pickle("tmp-hetero/"+str(w) + mode+".pickle")        
        towrite.to_csv(all_csv,header=False, index = False)

    [os.system(f"rm -r {x}") for x in ["tmp-hetero", "DataByWeek-hetero", "NumDataByWeek-hetero"]]
